...
export default class PhotoelectricschoolddetailComponent extends Component {
  ...
  renderComment(){
    ...
    for (var i = 0; i < 5; i++) {
      data.push(
        <View style={{paddingVertical:15,paddingHorizontal:12,backgroundColor:'#fff',borderBottomColor:'#d7d7d7',borderBottomWidth:0.5}} key={i}>
      ...
      <View style={styles.wrapper}>
        <BaseNavigationBar
            data={{
                ...
                rightbtn: {
                    type: BaseNavigationBar.TYPE._TEXT,
                    text:'编辑',
                    onClick: ()=> {
                        this.props.nav.push({
                          id: 'PublishDiary'
